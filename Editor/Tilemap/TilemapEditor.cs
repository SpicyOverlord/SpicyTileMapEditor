...
namespace SpicyTilemapEditor
{
    ...
    {
        [MenuItem("GameObject/SpicyTilemapEditor/Tilemap", false, 10)]
        ...
        private void RegisterTilesetEvents(Tileset tileset)
        {
            if (tileset == null) 
                return;
        ...
        private void UnregisterTilesetEvents(Tileset tileset)
        {
            if (tileset == null) 
                return;
        ...
        private void OnInspectorGUI_Map()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if(GUILayout.Button(new GUIContent("Reset", "Reset the CellSize to the default value in base to the Tileset properties: PixelsPerUnit and TileSize"), GUILayout.Width(50)))
                {
        ...
        private void DoMapSceneGUI()
        {
            if (m_toggleMapBoundsEdit)
            {
                ...
                var fmh_1042_178_638506880024437110 = Quaternion.identity; Vector3 vMinX = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MinGridX * m_tilemap.CellSize.x, m_tilemap.MapBounds.center.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1043_185_638506880024454390 = Quaternion.identity; Vector3 vMaxX = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2((m_tilemap.MaxGridX + 1f) * m_tilemap.CellSize.x, m_tilemap.MapBounds.center.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1044_178_638506880024457060 = Quaternion.identity; Vector3 vMinY = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x, m_tilemap.MinGridY * m_tilemap.CellSize.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1045_185_638506880024460220 = Quaternion.identity; Vector3 vMaxY = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x, (m_tilemap.MaxGridY + 1f) * m_tilemap.CellSize.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
            ...
            if (EditorWindow.mouseOverWindow == SceneView.currentDrawingSceneView)
            {
                ...
                if (tile != null)
                {
                    ...
                    for (int i = 0; i < handlesPos.Count; ++i)
                    {
                        ...
                        {
                            var fmh_1143_75_638506880024465720 = Quaternion.identity; handlePos = Handles.FreeMoveHandle(handlePos, 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
        ...
        bool DoToolBar()
        {
            ...
            Rect rTools = new Rect(
                44f, 
                4f, 
                Mathf.Max(Mathf.Max(Styles.Instance.toolbarBoxStyle.CalcSize(brushCoords).x, Styles.Instance.toolbarBoxStyle.CalcSize(selectedTileOrBrushId).x) + 4f, 180f), 
                64f);
        ...
        private void DisplayHelpBox()
        {
            ...
            // extend window to better fit the content
            Vector2 helpWindowSize = Styles.Instance.toolbarBoxStyle.CalcSize(helpContent);
            helpWindowSize.x += 21;
            
            Rect rHelpBox = new Rect(new Vector2(44f, 72f), helpWindowSize);
