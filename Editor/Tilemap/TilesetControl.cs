    ...
    public class TilesetControl
    {
        ...
        public bool Display()
        {
            ...
            float tileAreaHeight = (Tileset.VisualTileSize.y + visualTilePadding) *
                (1f + (m_visibleTileCount - 1f) / m_sharedData.tileViewRowLength) + 4f;
            ...
            EditorGUILayout.BeginHorizontal();
            string brushName = Tileset.FindBrush(Tileset.SelectedBrushId)?.name;
            if (brushName != null)
                EditorGUILayout.LabelField($"Brush Palette: ({brushName})", EditorStyles.boldLabel);
            else
            {
            ...
            if (tileRowLength <= 0)
                tileRowLength = 1;
            float fBrushesScrollMaxHeight = Screen.height / 4f;
                ...
                if (m_updateScrollPos) m_sharedData.brushesScrollPos = brushesScrollPos;
                {
                    ...
                    if (Tileset.Brushes != null)
                    {
                        GUILayout.Space((Tileset.VisualTileSize.y + visualTilePadding) *
                                        (1f + (Tileset.Brushes.Count - 1f) / tileRowLength));
            ...
            {
                // Scroll Moving Drag
                if (e.type == EventType.MouseDrag && e.button is 1 or 2)
                ...
                if (Tileset.Tiles != null)
                {
                    ...
                    int tileViewHeight = tileView != null
                        ...
                        : Tileset.Height;
                    int totalCount = ((tileViewWidth - 1) / m_sharedData.tileViewRowLength + 1) *
                    ...
                    for (int i = 0; i < totalCount; ++i)
                    {
                        ...
                        while (tile?.uv == default(Rect)) // skip invalid tiles
                        {
                            ...
                            //---
                        {
                            ...
                            if ((isLeftMouseReleased || isRightMouseReleased) && isInsideTileScrollArea &&
                                ...
                                && m_rTileScrollSize.Contains(e.mousePosition -
                                                              m_sharedData
                                                                  .tilesScrollPos)) // and it's inside the scroll area
                            {
                            ...
                            .Key /*&& m_startDragTileIdxRect.Key == m_endDragTileIdxRect.Key*/)
                    {
                        ...
                        HandlesEx.DrawRectWithOutline(rSelection, new Color(0f, 0f, 0f, 0.15f),
