...
namespace SpicyTilemapEditor
{
    ...
    {
        [MenuItem("Assets/Create/SpicyTilemapEditor/Brush/RandomBrush", priority = 50)]
        public static RandomBrush CreateAsset() => EditorUtils.CreateAssetInSelectedDirectory<RandomBrush>();
        ...
        public override void OnEnable()
        {
            ...
            m_randTileList.drawElementCallback += (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                SpicyTilemapEditor.TilesetBrush brush = m_brush.Tileset.FindBrush(brushId);
                if (brush)
                {
        ...
        {
            // add empty tile to brush, if no tiles are set
            // fixes "index out of range" exception when editing an empty brush
            if (m_brush.RandomTileList?.Count == 0)
            {
                m_brush.RandomTileList.Add(new RandomBrush.RandomTileData()
                {
            ...
            uint brushTileData = m_randTileList.index >= 0 && m_brush.RandomTileList.Count > 0
                ? m_brush.RandomTileList[m_randTileList.index].tileData
            ...
            EditorGUILayout.HelpBox("Please select a tile from list and then select a tile from tile selection window.",
                MessageType.Info);
