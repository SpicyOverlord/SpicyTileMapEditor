...
namespace SpicyTilemapEditor
{
    ...
    public class Tile
    {
        ...
        public int autotilingGroup = 0;
    }

    [Serializable]
    public class TileSelection
    {
        public List<uint> selectionData => m_tileIds;

        public int rowLength => m_rowLength;

        public int columnLength => 1 + (m_tileIds.Count - 1) / m_rowLength;

        [SerializeField] private int m_rowLength = 1;

        //NOTE: name remains for compatibility but now it contains tileData instead of only the id
        [SerializeField] private List<uint> m_tileIds;
        ...
        {
            List<uint> flippedTileIds = new List<uint>();
            ...
            for (int x = 0; x < rowLength; ++x)
            {
                ...
                flippedTileIds.Add(m_tileIds[idx]);
            }

            m_tileIds = flippedTileIds;
        }
    }
    ...
    public class TileView
    {
        ...
        public TileSelection tileSelection => m_tileSelection;
    ...
    public class Tileset : ScriptableObject
    {
        ...
        {
            if (tileId < 0 || m_tiles.Count <= tileId)
                return null;

            return m_tiles[tileId];
        }

        public void AddTileView(string tileViewName, TileSelection tileSelection, int idx = -1)
        {
            idx = idx >= 0 ? Mathf.Min(idx, m_tileViews.Count) : m_tileViews.Count;
            string viewName = tileViewName;
            int i = 1;
            while (m_tileViews.Exists(x => x.Name == viewName))
            {
                viewName = tileViewName + " (" + i + ")";
                ++i;
            }
                    ...
                    if (textureImporter != null)
                    {
                        ...
                        if (textureImporter.textureType == UnityEditor.TextureImporterType.Sprite)
                        {
                            if (textureImporter.spriteImportMode == UnityEditor.SpriteImportMode.Multiple)
                            {
                                ...
                                // before: "textureImporter.spritesheet" to get spriteData (now SpriteRect)
                                ...
                                if (spriteRects.Length >= 2)
                                {
                                    var spr0 = spriteRects[0];
                                    var spr1 = spriteRects[1];
                                    TilePxSize = spr0.rect.size;
                                ...
                                if (spriteRects.Length >= 2)
                                {
                                    ...
                                    if (m_tiles.Count == 0 || !UnityEditor.EditorUtility.DisplayDialog(
                                            "Import Sprite Sheet?",
                                        ...
                                        //---
                                    {
                                        ...
                                        {
                                            Rect rUV = new Rect(
                                                Vector2.Scale(spriteRect.rect.position, AtlasTexture.texelSize),
                                                Vector2.Scale(spriteRect.rect.size, AtlasTexture.texelSize));
                                            tileList.Add(new Tile() { uv = rUV });
                                            if (tileList.Count >= 2)
                                            {
                                                if (tileList[^2].uv.y == tileList[^1].uv.y)
                                                    continue;

                                                if (m_tilesetHeight == 1)
                                                {
                                                    m_tilesetWidth = tileList.Count - 1;
                                                    SlicePadding.y = spriteRects[tileList.Count - 2].rect.y -
                                                                     spriteRects[tileList.Count - 1].rect.yMax;
                                                }
...
#endif
                                        {
                                            for (int i = 0; i < m_tiles.Count && i < tileList.Count; ++i)
                                            {
                                                tileList[i].collData = m_tiles[i].collData;
                                                tileList[i].paramContainer = m_tiles[i].paramContainer;
                                                tileList[i].prefabData = m_tiles[i].prefabData;
                                            }
                                        }
