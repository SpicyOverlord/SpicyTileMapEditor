...
namespace SpicyTilemapEditor.PathFindingLib
{
    public class IPathNode
    {
        ...
        public virtual float GetNeighborMovingCost(int neighborIdx) { return 1f; }
            ...
            else
            {
                ...
                do
                {
                    ...
                    for (int i = 0, s = curNode.Value.GetNeighborCount(); i < s; ++i)
                    {
                        ...
                        IPathNode neighborNode = curNode.Value.GetNeighbor(i);
                        float movingCost = curNode.Value.GetNeighborMovingCost(i);
                        bool isNeighborNodePassable = movingCost != k_InfiniteCostValue && neighborNode.IsPassable();
                        if (
                            neighborNode.closeComputeId != m_computeId && // if closeList does not contains node
                           ...
                           ) 
                        {
                            ...
                            if (neighborNode.openComputeId != m_computeId // if openList does not contains node
                                && newDist <= findingParams.maxPathDistance // if distance is under limits
                                ) 
                            {
                                m_openList.AddLast(neighborNode); neighborNode.openComputeId = m_computeId;
                                neighborNode.ParentNode = curNode.Value;
                                neighborNode.Cost = newCost;
                                neighborNode.Score = neighborNode.Cost + neighborNode.GetHeuristic();
                                neighborNode.Distance = newDist;
                                
                                if (neighborNode == findingParams.endNode)
                                {
                                    curNode.Value = neighborNode;
                            ...
                            else if (newCost < neighborNode.Cost)
                            {
